...
The declaration generator's public-only mode uses the new raw inventory. It retains direct `export =` callable generation by supplying verified module identity to the unchanged legacy builder. It **does not pretend** the builder supports object members, aliases or static member paths: those observed public functions remain in trace evidence but are withheld from generated declarations with `UNSUPPORTED_PUBLIC_EXPORT_PATH`. Unrelated helpers receive `FILTERED_INTERNAL_API`. Historical raw files lacking the inventory still use the old boundary with `LEGACY_PUBLIC_BOUNDARY_UNVERIFIED`, making this fallback visible rather than silently certified.
...
The normalized `functionId` identifies a source function, while `exportPaths` names independent routes to it. The legacy declaration builder cannot yet emit correct object/re-export declarations, so **public trace recall and generated declaration recall have different supported denominators**. The next Line A step is a builder/export-identity extension with a validated object-member fixture, then a fixed-version development package pilot comparing README-only, tests-only, unfiltered union and filtered union. Keep eligibility, exclusions, runtime image/lock/reference provenance and human-reviewed reference declarations separate. This audit alone provides no empirical quality-improvement claim.
