...
Each package is extracted in a separate worker with a 1 GB V8 heap, a two-minute deadline and an explicit 256 MB file-output cap. Shards use temporary-file transport rather than a bounded stdout pipe; diagnostic stdout/stderr is capped at 1 MB, and temporary files are removed after collection. This replaces the original 64 MB stdout transport that failed on three large packages in the baseline. Failed/missing entries retain an explicit zero-row failure shard and make the census command nonzero; they are not negative research results. Resume retries failed shards. Successful package shards avoid repeated compilation. An implementation hash and final input/implementation recheck prevent unnoticed rule/input drift. Aggregation streams shards to JSONL/CSV rather than holding all declarations/programs in memory; distinct-ID sets still consume memory proportional to the corpus. For unusually large corpora increase the parent Node heap deliberately, not worker limits silently. Recursive namespace alias cycles are cut along the current ancestor path and inventoried as exclusions; independent aliases remain distinct public paths. Acyclic depth/export budgets remain in force.
...
Class tags are non-exclusive: identity, container-element, value-to-container, multiple-arguments, constrained-generic, higher-order, overload-or-discriminated and unsupported. Their sum need not equal total declarations. Container-element and value-to-container require outer container routes: array occurrences beneath callback/function boundaries cannot supply those tags. A source review found four such overbroad baseline classifications; the baseline stays preserved and changed rules require a fresh run. An identity tag records a positional relationship (possibly a union projection), not automatically an exact `<T>(T)->T` sealing contract. `firstOrderPotential` is only an initial syntactic shortlist with no enclosing formal parameters or unsupported forms; it does not establish executable exports or existing kernel support. Every generic candidate needs source/client/runtime review before becoming a pilot API.
