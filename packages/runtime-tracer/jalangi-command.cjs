...
var instUtil = require('./vendor/jalangi2/src/js/instrument/instUtil');
...
acorn = require('acorn-legacy');
esotope = require('esotope');
require('./vendor/jalangi2/src/js/headers').headerSources.forEach(function (header) {
  require('./vendor/jalangi2/' + header);
});
...
var logFile = path.join(process.env.TRACE_WORK_DIR || process.cwd(), 'jalangi.log');
...
Module._extensions['.js'] = function (module, filename) {
  ...
  J$.traSourceMaps = J$.traSourceMaps || {};
  J$.traSourceMaps[filename] = JSON.parse(transformed.sourceMapText);
