...
function analyzePackage(root, directory, entryOption) {
  ...
  if (!entryOption) {
    ...
    const missing = results.filter(result => result.package.status === 'missing-entry');
    const status = missing.length === results.length ? 'missing-entry' : missing.length ? 'partial-entry' : diagnostics.length ? 'extracted-with-diagnostics' : 'extracted';
    return {package: {...first, entries: results.map(result => ({entry: result.package.entry, subpath: result.package.subpath, condition: result.package.condition, status: result.package.status, entrySha256: result.package.entrySha256})), entryCoverage: {selected: results.length, extracted: results.length - missing.length, missing: missing.map(result => ({entry: result.package.entry, condition: result.package.condition, subpath: result.package.subpath}))}, diagnosticCount: results.reduce((total, result) => total + (result.package.diagnosticCount || 0), 0), retainedDiagnosticCount: diagnostics.length, diagnostics, sourceNotices: Object.assign({}, ...results.map(result => result.package.sourceNotices || {})), exclusions: results.flatMap(result => result.package.exclusions), status}, rows: [...new Map(results.flatMap(result => result.rows).map(row => [row.id, row])).values()].sort((left, right) => left.id.localeCompare(right.id))};
  }
